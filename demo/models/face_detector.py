# tvogiannou: modified version to support async mxnet calls on GPU 
# original found at https://github.com/deepinsight/insightface/blob/master/python-package/insightface/model_zoo/face_detection.py
...
class FaceDetectorHandler:
    def __init__(self, cfg, ctx_id=0):
        self.limit = cfg.IMAGE_SIZE
        self.detector = FaceDetector(param_file=cfg.PRETRAINED)
        self.detector.prepare(ctx_id=ctx_id)

    def dispatch_gpu(self, img):
        det_scale = 1.0
        if min(img.shape[0:2]) > self.limit:
            det_scale = float(self.limit) / min(img.shape[0:2])
        scale = self.detector.detect_async(img, scale=det_scale)
        return scale

    def get_results(self, scale):
        bboxes, lms = self.detector.get_async_results(scale=scale)
        if bboxes.shape[0] == 0:
            return None, None
        return bboxes, lms
...
class FaceDetector:
    def __init__(self, param_file, default_image_size=(480, 640)):
    ...
    def detect_async(self, img, scale=1.0):
        """Asyncronous version of detect"""
    ...
    @Timer(name='Assigner', fps=True, pprint=False)
    def get_async_results(self, threshold=0.5, scale=1.0):
        ...
        for _idx, s in enumerate(self._feat_stride_fpn):
            # for _idx,s in reversed(list(enumerate(self._feat_stride_fpn))):
            _key = 'stride%s' % s
            stride = int(s)
            ...
            # print("{} at {}".format((t1 - t0)*1000, _idx))
